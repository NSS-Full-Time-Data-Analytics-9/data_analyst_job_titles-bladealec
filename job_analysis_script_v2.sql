...
GROUP BY title;
-- 4 different titles, and 3 have 'data'
...
SELECT COUNT(domain) AS domain_count
FROM data_analyst_jobs
...
-- Banks and Finanicial Services	(61)
-- Consulting and Business Services	(57)


-- Different way to approach Bonus; Seems to coincide with the "Which three industries are in the top 4 on this list?" question.


SELECT domain, COUNT(domain) AS domain_count, skill
FROM data_analyst_jobs
WHERE skill ILIKE '%sql%'
	AND days_since_posting > (3*7)
	AND domain IS NOT NULL
